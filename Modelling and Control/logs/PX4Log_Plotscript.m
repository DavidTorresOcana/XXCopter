...
addpath(genpath('log files'),genpath('kmltoolbox_v2.7'))

% ************************************************************************
% SETTINGS
% ************************************************************************

%% Get the log file decoded with Python script
[FileName,PathName] = uigetfile({'*.px4log; *.bin',...
 'Log Files (*.px4log;*.bin)';     '*.*',  'All Files (*.*)'},' Select the log file you want to decode');

strr = strsplit(FileName,'.');

data_file = [strr{1},'.csv'];
...
% KML of GPS
% ************************************************************************
k = kml('kml of Log file'); %create an kmltoolbox object
f = k.createFolder('kml.plot');
f.plot3(sysvector.GPS_Lon, sysvector.GPS_Lat, sysvector.GPS_Alt, 'altitudeMode','absolute','lineWidth',5,'lineColor','FF00FF00', 'name','plot3 test');
...
function ImportPX4LogData()
    ...
    if ~exist('C:/Python27')
        errordlg(' You dont have Python installed. Please install Python27 in your computer')
        return
    end
    
    if not(exist(data_file, 'file'))
        s = system( sprintf('C:/Python27/python sdlog2_dump.py "%s" -f "%s" -t"%s" -d"%s" -n"%s"', arg1, data_file, time_field, delim, csv_null) );
    end
