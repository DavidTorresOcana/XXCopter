...
const char *dev_rgbled = RGBLED_DEVICE_PATH;
const char *dev_pwm = PWM_OUTPUT_DEVICE_PATH;
...
/* Position in local NED frame 
	local_pos.x;				/**< X position in meters in NED earth-fixed frame 
	local_pos.y;				/**< X position in meters in NED earth-fixed frame 
	local_pos.z;				/**< Z position in meters in NED earth-fixed frame (negative altitude) 
	// Velocity in NED frame 
	local_pos.vx; 				/**< Ground X Speed (Latitude), m/s in NED 
	local_pos.vy;				/**< Ground Y Speed (Longitude), m/s in NED 
	local_pos.vz;				/**< Ground Z Speed (Altitude), m/s	in NED 
	// Heading 
	local_pos.yaw;
			*/
   struct battery_status_s               bat_status;
           
//   struct vehicle_attitude_setpoint_s        attitude_sp;		/**< vehicle attitude setpoint */
//   struct vehicle_local_position_setpoint_s	local_pos_sp;                   /**< vehicle local position setpoint */
/*
	local_pos_sp.x;		/**< in meters NED			  		
	local_pos_sp.y;		/**< in meters NED			  		
	local_pos_sp.z;		/**< in meters NED			  		
	local_pos_sp.yaw;		/**< in radians NED -PI..+PI  		
*/

//   struct position_setpoint_triplet_s		pos_sp_triplet;             /**< vehicle global position setpoint triplet */
/*  Comming from Navigator::publish_position_setpoint_triplet
	pos_sp_triplet.x;			/**< local position setpoint in m in NED 
	pos_sp_triplet.y;			/**< local position setpoint in m in NED 
	pos_sp_triplet.z;			/**< local position setpoint in m in NED 
	*/
//   struct vehicle_global_velocity_setpoint_s	global_vel_sp;              /**< vehicle global velocity setpoint */
  ...
  struct {
   ...
   GCS_comms_pointers.X_comm = param_find("X_COMMANDED");
   GCS_comms_pointers.Y_comm = param_find("X_COMMANDED");
   GCS_comms_pointers.Alt_comm = param_find("X_COMMANDED");
   
   GCS_comms_pointers.Vx_yaw_comm = param_find("VX_YAW_COMMANDED");
   GCS_comms_pointers.Vy_yaw_comm = param_find("VY_YAW_COMMANDED");
   GCS_comms_pointers.Vz_yaw_comm = param_find("VZ_YAW_COMMANDED");
   
   GCS_comms_pointers.phi_comm = param_find("PHI_COMMANDED");
   GCS_comms_pointers.theta_comm = param_find("THETA_COMMANDED");
   GCS_comms_pointers.head_comm = param_find("HEADING_COMMANDED");
   
   GCS_comms_pointers.flight_mode_request = param_find("FLIGHT_MODE_REQUEST"); 
  /** Getting parameters and data from the GCS
   * It can be used to pass desired parameters like altitude,x,y position, etc from the GCS to Simulink directly
   *
   * https://pixhawk.org/dev/parameters
   *
   * PARAM_DEFINE_FLOAT(ALT_COMMANDED, 1.0f);
   * param_t param_pointer = param_find("ALT_COMMANDED");
   *
   * float Alt_comm;
   * param_get(param_pointer,&Alt_comm);
   *
   * simulink_app_U.Alt_comm = Alt_comm;
   **/
