...
% Load bus objs: In case of bus changes, use Create_bus_obj script
load('bus_objs.mat')
...
throtle_1= 0*0.745; % Those are suposed to be the trimed state ones
eta_23 = deg2rad(-1.3);
throtle_23 = throtle_1/cos(eta_23);
% Trim_Cost = TrimCostFunction(100*[throtle_1,throtle_23,eta_23])

% Hover trim 
Hover_throtle_1= 0.745; % Those are suposed to be the trimed state ones
Hover_eta_23 = deg2rad(-1.3);
Hover_throtle_23 = Hover_throtle_1/cos(Hover_eta_23);
%% CA settings
...
Max_Thrust = rho*pi*R^2*(Max_Omega*R)^2*0.01; % Aprox Conservative

A_z_max = Max_Thrust*3/m;

A_l_max_x = sqrt( (A_z_max)^2  -  g^2  );
A_l_max_y = g*tand(15);
