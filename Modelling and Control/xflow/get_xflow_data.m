...
% xflow_data.speed      = Velocidad de ensayo
...
% xflow_data.Fx         = Valor promedio de Fx en ejes cuerpo
% xflow_data.Fy         = Valor promedio de Fy en ejes cuerpo
% xflow_data.Fz         = Valor promedio de Fz en ejes cuerpo
% xflow_data.Mx         = Valor promedio de Mx en ejes cuerpo
% xflow_data.My         = Valor promedio de My en ejes cuerpo
% xflow_data.Mz         = Valor promedio de Mz en ejes cuerpo
%
...
xflow_data.time_start    = vector_time(counter_time);
xflow_data.time_end      = vector_time(end);

xflow_data.Fx            = mean (vector_F_FA(counter_time:end,1));
xflow_data.Fy            = mean (vector_F_FA(counter_time:end,2));
xflow_data.Fz            = mean (vector_F_FA(counter_time:end,3));
xflow_data.Mx            = mean (vector_M_FA(counter_time:end,1));
xflow_data.My            = mean (vector_M_FA(counter_time:end,2));
xflow_data.Mz            = mean (vector_M_FA(counter_time:end,3));

xflow_data.std.Fx        = std (vector_F_FA(counter_time:end,1))/abs(xflow_data.Fx);
xflow_data.std.Fy        = std (vector_F_FA(counter_time:end,2))/abs(xflow_data.Fy);
xflow_data.std.Fz        = std (vector_F_FA(counter_time:end,3))/abs(xflow_data.Fz);
xflow_data.std.Mx        = std (vector_M_FA(counter_time:end,1))/abs(xflow_data.Mx);
xflow_data.std.My        = std (vector_M_FA(counter_time:end,2))/abs(xflow_data.My);
xflow_data.std.Mz        = std (vector_M_FA(counter_time:end,3))/abs(xflow_data.Mz);
